fit_mod <- function(df, model, name = "EST", EANM = T ,Y = "HT", lnmodel=T){
  
  MODEL <- as.formula(model) # transformar em formula
  ...
  # estimar apenas para arvores nao medidas, se o usuario quiser
    if(EANM==T){ 
      df$EST <- ifelse( is.na(df[["HT"]])  , EST, df[["HT"]]  )  
  ...
  # remove as variaveis usadas na estimacao (as variaveis do modelo, e os betas)
  df[ c(all.vars(MODEL) ,  paste("b", 0:length(VARSX), sep="" )  ) ] <- NULL
